...
rule LLTTCS_DOCM_Dropper_DecryptedStrings
{
    ...
    condition:
        ($FileUrl1*) and ($FileUrl2*) and ($XMLHTTP1*) and ($XMLHTTP2*) and $GET and ($ADODB1*) and ($ADODB2*) and ($FilePath1*) and ($FilePath2*) and ($Command*) and ($Shell*) and ($ErrMsg1*) and ($ErrMsg2*)
}
